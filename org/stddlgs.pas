...
interface
  uses
       Objects, Drivers, Views, Dialogs, Dos;
  ...
  type
       ...
       TFileDialog = object (TDialog)
         ...
         constructor Init (AWildCard: TWildStr; ATitle: String;
                           InputName: String; AOptions: Word;
                           HistoryId: Byte);
...
function IsDir (var S: String): Boolean;
  ...
  begin
    ...
    if DosError=0 then IsDir:=SR.Attr and Directory<>0
                  else IsDir:=False;
  end;
        ...
        if Event.CharCode<>#0 then
          begin
            ...
            if Event.KeyCode=kbBack then
              ...
              if (Event.CharCode='.') then SearchPos:=Pos('.',CurString)
              else
                begin
                  Inc(SearchPos);
...
procedure TFileList.SetData (var Rec);
  begin
    with PFileDialog(Owner)^ do Self.ReadDirectory(Directory^+WildCard);
  end;
...
procedure TFileInfoPane.Draw;
  ...
  begin
    ...
    FormatStr(Strng,FmtId,Params);
    MoveStr(B,Strng,Color);
...
constructor TFileDialog.Init (AWildCard: TWildStr; ATitle: String;
  ...
  begin
    ...
    Insert(FileList);
    R.Assign(2,5,8,6);
    Control:=New(PLabel,Init(R,'~F~ile',FileList));
    Insert(Control);
  ...
  function CheckDirectory (var S: PathStr): Boolean;
    begin
      if not PathValid(S) then
        begin
          MessageBox('Invalid file name or directory.',nil,mfError+mfOkButton);
          FileName^.Select;
          ...
          begin
            if IsWild(FName) then
              ...
              if IsDir(FName) then
                ...
                if ValidFileName(FName) then Valid:=True
                else
                  begin
